...
function getNumbersBeforeHashMap(pageOrderingRules) {
  const hashMap = {};
  ...
  for (const rule of pageOrderingRules) {
    ...
    hashMap[before][after] = true;
  }

  return hashMap;
}

function getNumbersAfterHashMap(pageOrderingRules) {
  const hashMap = {};

  for (const rule of pageOrderingRules) {
    const [after, before] = rule.split("|")
...
function main() {
  const numbersBeforeHashMap = getNumbersBeforeHashMap(pageOrderingRules);
  const numbersAfterHashMap = getNumbersAfterHashMap(pageOrderingRules);
  let total = 0;

  for (const printCommand of printCommands) {
    try {
      for (let i = 0; i < printCommand.length; i++) {
        for (let j = 0; j < printCommand.length; j++) {
          if (j === i) {
            continue;
          }
          
          if (i < j && numbersAfterHashMap[printCommand[i]]?.[printCommand[j]]) {
            throw new Error("Number found after it was expected")
          }

          if (i > j && numbersBeforeHashMap[printCommand[i]]?.[printCommand[j]]) {
            throw new Error("Number found after it was expected")
          }
        }
      }

      total += printCommand[Math.floor(printCommand.length / 2)]
    } catch (err) {
      continue;
    }
  }
